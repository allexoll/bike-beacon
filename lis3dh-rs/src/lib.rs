    ...
    E: Debug,
{
    ...
    pub fn config_interrupt(&mut self) -> Result<(), Error<E>> {
        ...
        self.write_register(Register::CTRL3, I1_IA1)?;

        // enable interrupt 1 pin latched
        self.write_register(Register::CTRL5, 1<< 3);
        // configure threshold
        self.write_register(Register::INT1_THS, 0x16)?;

        // configure int duration
        self.write_register(Register::INT1_DURATION, 0x02)?;


        // configure movement detection with 3 axis high OR'ed
